...
class Item(pg.sprite.Sprite):  # описание спрайтов окна
    def __init__(self, image, store_rect, ind, type):
        super().__init__()
        self.image = image
        self.rect = self.image.get_rect()
        self.rect = (self.rect.midleft[0] + int(config.WIDTH * 4 / 21) * ind + 500, self.rect.midleft[1] + 120)

        self.amount = 0
        self.type = type
        self.button_image = pg.image.load("images/buttom_hiring.png")
        ...
        self.button_create_rect = self.image.get_rect()
        self.button_create_rect.topleft = (self.rect[0], self.rect[1])
        # self.button_rect = self.button_image.get_rect()

    def draw(self, screen, amount):
        screen.blit(self.image, self.rect)
        screen.blit(self.button_image, self.button_rect)
        text_rect = (self.rect[0] + 80, self.rect[1] - 60)
...
class Store(pg.sprite.Sprite):  # класс окна замка
